...
use cosmwasm_std::{from_binary, to_json_binary, Addr, Api, Attribute, Coin, CosmosMsg, DepsMut, Empty, Response, Uint128, WasmMsg};
...
fn setup_contract(deps: DepsMut<'_>) -> Cw721Contract<'static, Extension, Empty> {
    ...
    let info = mock_info(MINTER, &[Coin {
        denom: "utoken".to_string(),
        amount: Uint128::new(1000000),
    }]);
...
fn minting() {
    ...
    // Test with incorrect payment first
    let random = mock_info("random", &[]);
    let err = contract
        .execute(deps.as_mut(), mock_env(), random, mint_msg.clone())
        .unwrap_err();
    assert_eq!(err, ContractError::IncorrectPayment {});

    // Now test with correct payment but unauthorized minter
    let unauthorized = mock_info("unauthorized", &[Coin {
        denom: "utoken".to_string(),
        amount: Uint128::new(1000000),
    }]);
    let err = contract
        .execute(deps.as_mut(), mock_env(), unauthorized, mint_msg.clone())
        .unwrap_err();
    assert_eq!(err, ContractError::Unauthorized {});

    // Finally, test with correct payment and authorized minter
    let minter = mock_info(MINTER, &[Coin {
        denom: "utoken".to_string(),
        amount: Uint128::new(1000000),
    }]);
    let res = contract
    ...
    assert_eq!(
        res.attributes,
        vec![
            Attribute::new("action", "mint"),
            Attribute::new("minter", MINTER),
    ...
    let allowed = mock_info(MINTER, &[Coin {
        denom: "utoken".to_string(),
        amount: Uint128::new(1000000),
    }]);
    let err = contract
...
fn query_tokens_by_owner() {
    ...
    // Use Addr type for addresses
    let demeter_addr = deps.api.addr_validate(&demeter).unwrap();
    let ceres_addr = deps.api.addr_validate(&ceres).unwrap();

    // all tokens by owner
    let tokens = contract
        .tokens(deps.as_ref(), demeter_addr.to_string(), None, None)
        .unwrap();
    assert_eq!(&by_demeter, &tokens.tokens);
    let tokens = contract.tokens(deps.as_ref(), ceres_addr.to_string(), None, None).unwrap();
    assert_eq!(&by_ceres, &tokens.tokens);

    // paginate for demeter
    let tokens = contract
        .tokens(deps.as_ref(), demeter_addr.to_string(), None, Some(1))
        .unwrap();
    assert_eq!(&by_demeter[..1], &tokens.tokens[..]);
    let tokens = contract
        .tokens(deps.as_ref(), demeter_addr.to_string(), Some(by_demeter[0].clone()), Some(3))
        .unwrap();
    assert_eq!(&by_demeter[1..], &tokens.tokens[..]);
}
...
fn max_mints_limit() {
    ...
    // Mint up to the max_mints limit
    for i in 1..=100 {
        let token_id = format!("token{}", i);
        let mint_msg = ExecuteMsg::Mint(MintMsg::<Extension> {
            token_id: token_id.clone(),
            owner: String::from("collector"),
            token_uri: None,
            extension: None,
        });
    ...
    // Attempt to mint one more token beyond the limit
    let mint_msg = ExecuteMsg::Mint(MintMsg::<Extension> {
        token_id: "token101".to_string(),
        owner: String::from("collector"),
        token_uri: None,
        extension: None,
    });
    ...
    let err = contract
        .execute(deps.as_mut(), mock_env(), minter, mint_msg)
        .unwrap_err();
    assert_eq!(err, ContractError::MaxMintsReached {});
}

#[test]
fn mint_price_requirement() {
    let mut deps = mock_dependencies();
    let contract = setup_contract(deps.as_mut());

    let token_id = "expensive_token".to_string();
    let mint_msg = ExecuteMsg::Mint(MintMsg::<Extension> {
        token_id: token_id.clone(),
        owner: String::from("collector"),
        token_uri: None,
    ...
    // Attempt to mint without sending enough funds
    let minter = mock_info(MINTER, &[Coin {
        denom: "utoken".to_string(),
        amount: Uint128::new(999999), // 1 less than required
    }]);
    let err = contract
        .execute(deps.as_mut(), mock_env(), minter, mint_msg.clone())
        .unwrap_err();
    assert_eq!(err, ContractError::IncorrectPayment {});

    // Mint with correct amount of funds
    let minter = mock_info(MINTER, &[Coin {
        denom: "utoken".to_string(),
        amount: Uint128::new(1000000),
    }]);
    ...
    let owner = contract
        .owner_of(deps.as_ref(), mock_env(), token_id.clone(), true)
        .unwrap();
    assert_eq!(owner.owner, String::from("collector"));
}

#[test]
fn mint_with_incorrect_denom() {
    let mut deps = mock_dependencies();
    let contract = setup_contract(deps.as_mut());

    let token_id = "wrong_denom_token".to_string();
    let mint_msg = ExecuteMsg::Mint(MintMsg::<Extension> {
        token_id: token_id.clone(),
        owner: String::from("collector"),
        token_uri: None,
    ...
    // Attempt to mint with incorrect denom
    let minter = mock_info(MINTER, &[Coin {
        denom: "wrongtoken".to_string(),
        amount: Uint128::new(1000000),
    }]);
    let err = contract
        .execute(deps.as_mut(), mock_env(), minter, mint_msg)
